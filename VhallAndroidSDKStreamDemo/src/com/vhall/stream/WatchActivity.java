...
import com.vhall.netbase.constants.ZReqEngine.Attend;
...
public class WatchActivity extends Activity {
	...
	public void initView(){
		...
		controller = new VhallMediaController(this);
		controller.newInstance(mLiveCallBack, false);
...
//		url = "rtmp://cnrtmplive01.e.vhall.com/vhall/906807302";
		...
		boolean isConnection = controller.connectRecvStar(url);
		if (isConnection){
			mProcessDialog.show();
		}else{
			showAlert("开始观看失败");
		...
		public void notifyEvent(int resultCode, String content) {
			if (handler != null) {
				try {
					...
					message.obj = content;
					Log.v(TAG , " what ====" + message.what + "obj =====" + message.obj);
	...
	private void initWH(int width, int height) {
		...
		if(width != videoWidth || height != videoHeight ){
			mFrameLayout.removeView(glSurfaceview);
			mFrameLayout.addView(glSurfaceview);
		}
